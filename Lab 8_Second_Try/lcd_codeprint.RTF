...
char checkBusy()
{
    ...
    //Config PORTE to inputs
    // Store the current tri-state first though
    int TrisETemp = TRISE;
    TRISE = TRISE | 0x00FF;

    // Set RW and RS to request a read
    RW  = 1;
    RS = 0;

    //According to the data sheet, we need to wait for 60 ns
    //At instruction frequency of 16 MHz, each instrustion is 62.25 ns
    Nop();
    // Set E to high to start the read
    E = 1;

    //According to the data sheet, we need to wait for 190 ns
    //At instruction frequency of 16 MHz, each instrustion is 62.25 ns
    // 190 / 62.25 = 3.05. Therefore we should wait for 4 instructions to be safe
    Nop();
    Nop();
    Nop();
    Nop();
    ...
    // Read the PORTE bus
    // According to datasheet, the busy bit is located on the last bit of the data
    Result = PORTE & 0x80;
