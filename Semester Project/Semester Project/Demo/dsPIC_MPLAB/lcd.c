...
* Bill Barlow 		03/03/11  added check busy routine
* Bill Barlow		04/05/12  added code to initialize port B 
*							  pin 15 by turning off the analog function
*
*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
*
* ADDITIONAL NOTES:
...
char check_busy(void)			 //function to see of LCD is busy
{
	char Data_Byte;
	TRISE |= 0x00FF;			//make the data port an input
	RW=1;
	RS=0;
	Nop();						//62.5 ns delay to meet tas 60 ns setup time
	E =1;
	//max time to valid data is 190 ns using 16 MHZ inst freg
	//4 nops is 250 ns
	Nop();
	Nop();
	Nop();
	Nop();
	Data_Byte = PORTE & 0x80;
	E=0;
	TRISE &= 0xFF00;
	return(Data_Byte);		
}
